...
export async function searchVideos(query: string): Promise<Video[]> {
  try {
    ...
    const response = await fetch(`https://www.googleapis.com/youtube/v3/search?part=snippet&q=${encodeURIComponent(enhancedQuery)}&maxResults=12&type=video&videoDuration=long&relevanceLanguage=en&videoCategoryId=27&key=${API_KEY}`);
    ...
    const detailsResponse = await fetch(`https://www.googleapis.com/youtube/v3/videos?part=contentDetails,statistics,snippet&id=${videoIds}&key=${API_KEY}`);
